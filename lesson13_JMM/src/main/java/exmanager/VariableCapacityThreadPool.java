...
public class VariableCapacityThreadPool {
    ...
    private final List<Runnable> notStartingTask;//список не запушенных задач
    ...
    private volatile boolean isShutdown = true;//флаг полной остановки пула,  потоки сразу же завершают работу при старте
    ...
    public VariableCapacityThreadPool(String name) {
        this(name, 0, Integer.MAX_VALUE);
    }

    public VariableCapacityThreadPool(String name, int minCountThread) {
        this(name, minCountThread, minCountThread);
    }

    public VariableCapacityThreadPool(String namePool, int minCountThread, int maxCountThread) {
        ...
        this.notStartingTask = new CopyOnWriteArrayList<>();
...
//                        Runnable runnable = queueTask.poll();//извлечение задачи из очереди
    ...
    //запуск пула
    public void startPool() throws IllegalStateException {
        if (!isShutdown)
